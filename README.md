...
正在推进一个可测量的 [Eco 低功耗最小闭环](docs/low-power-eco-demo-plan.md)：先建立整机功耗基线，再验证 RTC 唤醒、STOP 恢复和 OTA 回归。**M1（RTC → STOP → 唤醒探针）已完成**，产物见 [`lowpower-probe/`](lowpower-probe/) 与[实现记录](docs/low-power-eco-demo-plan.md)第 3.4 节；⚠️ 探针电流与 57 mA Normal 基线不可比（探针不初始化任何外设），**整机省电成果（M2/M3）尚未验证，未实测前不宣称**。
